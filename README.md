...
All newer ESP32 boards comes with a built in [USB Serial/JTAG Controller](https://docs.espressif.com/projects/esp-idf/en/latest/esp32c3/api-guides/usb-serial-jtag-console.html). 
In many cases the USB port your ESP32 board, is connected directly to the USB Serial/JTAG Controller on the chip, in that case you wont be able connect over UART, unless you have a external USB to Serial chip connected to the UART pins on the board.

If your board does not have a dedicated USB to serial chip, you can change the console output to USB_SERIAL_JTAG.
