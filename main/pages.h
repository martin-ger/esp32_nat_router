...
var formSubmitted = qs.indexOf('ap_ssid=') !== -1 || (qs.indexOf('ssid=') !== -1 && qs.indexOf('password=') !== -1) || qs.indexOf('staticip=') !== -1 || qs.indexOf('reset=') !== -1 || qs.indexOf('disable_interface=') !== -1 || qs.indexOf('pcap_mode=') !== -1 || qs.indexOf('pcap_snaplen=') !== -1;\
if (formSubmitted) {\
...
<small>All fields saved to NVS. Device reboots after saving.</small>\
</form>\
<div style='margin-top: 1.5rem; text-align: center;'>\
<a href='/' style='padding: 0.6rem 1.5rem; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #fff; border: none; border-radius: 8px; text-decoration: none; font-size: 0.9rem; font-weight: 600;'>Home</a>\
</div>\
